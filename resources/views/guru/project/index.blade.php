        ...
        <div class="content">
            <div class="container-fluid">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('guru.update.project') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                <input type="hidden" name="id" value="{{ $project?->id ?? '' }}" id="">
                                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12 mt-3">
                                    <label for="">Embed Worksheet</label>
                                    {{-- <textarea name="body" class="editor" style="width: 100%;">{{ old('body', $project?->body) }}</textarea>
                                    @error('body')
                                        <small style="color: red;">
                                            {{ $message }}
                                        </small>
                                    @enderror --}}
                                    <textarea name="body" style="width: 100%;" rows="10">{{ old('body', $project?->body) }}</textarea>
                                    @error('body')
                                        <small style="color: red;">
                                            {{ $message }}
                                        </small>
